...
### 책 정보(제목 · 저자 · 전체 쪽수) 인라인 수정

- 등록 시 자동 채워진 제목·저자·전체 쪽수(검색 결과 오선택, 직접 입력 오타, 기본값 150쪽
  등)가 실제 책과 다를 수 있는데, 예전엔 등록 후 고칠 방법이 없었음.
- 처음엔 저자처럼 필드마다 따로 연필 아이콘을 뒀는데(제목/저자/전체 쪽수 각각), 카드 하나에
  연필이 3개나 반복돼 산만하다는 피드백으로 **연필 아이콘 하나**로 통합함. 제목 옆 연필
  클릭 → 제목·저자·전체 쪽수 입력 필드 3개 + 저장/취소 버튼이 함께 나타남.
- 저장 시 `handleInfoSave`가 바뀐 필드만 골라 해당 Server Action을 호출(`updateTitle`,
  `updateAuthor`, `updateTotalPages`) — 안 바뀐 필드까지 매번 쓰지 않음. 특히 전체 쪽수는
  현재 페이지 clamp·자동 완독 처리가 딸려 있어 불필요한 호출을 피하는 게 중요.
- 전체 쪽수가 없으면 진행률 줄에 "미입력"으로 표시하고 진행률 바 대신 안내 문구를 보여줌.
